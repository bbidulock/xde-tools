...
sub new_from_entry {
    ...
    my @icons = ( $entry->{Icon} );
    my $mime = XDE::Desktop->get_mime_type($path) if $path;
    push @icons, @{XDE::Desktop->get_icons($mime)} if $mime;
...
#   $icon->set_tooltip_text($label);
    ...
    $but->set_size_request(&XDE::Desktop::ICON_WIDE,&XDE::Desktop::ICON_HIGH);
#   $but->set_relief('none');
    ...
    my $n_chars = int((&XDE::Desktop::ICON_WIDE+8)/8);
...
sub create2 {
    ...
    $but->set_size_request(&XDE::Desktop::ICON_WIDE,&XDE::Desktop::ICON_HIGH);
    $but->set_alignment(&XDE::Desktop::ICON_WIDE,&XDE::Desktop::ICON_HIGH);
    $but->set_border_width(0);
    $but->set_relief('none');
...
sub open_with {
	...
	if (my $mime = $self->{mime}) {
	    my ($apps,$subs) = XDE::Desktop->get_apps_and_subs($mime);
	    if (@$apps or @$subs) {
		my $path = $self->{path};
